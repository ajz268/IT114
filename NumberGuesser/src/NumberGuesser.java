	...
	public static void main(String[] args) {
		...
		try (Scanner input = new Scanner(System.in);) {
			...
			while (input.hasNext()) {
				...
				if (message.equals("ADDSTRIKE")) {
					maxStrikes += 1;
					System.out.println("Cheat code! You've added a strike.");
					System.out.println("You now have " + maxStrikes + " total strikes.");
				}
				int guess = -1;
				try {
				...
				if (guess > -1) {
					...
					if (guess == number) {
						...
						strikes++;
						System.out.println("Strikes remaining: " + strikes + " out of " + maxStrikes); // added strike
																										// counter
						if (guess > number) {
							System.out.println("Hint: Choose a lower number!");
						}
						if (guess < number) {
							System.out.println("Hint: Choose a higher number!");
						}
